...
public class RestaurantOrder implements Order {
    private CircularList<Dish> order;

    public RestaurantOrder() {
        order = new CircularList<>(new DishComparator());
    }

    public RestaurantOrder(Item[] newItems) {
        for (Item item : newItems) {
            order.addNode((Dish)item);
        }
    }
    ...
    public boolean remove(Item item) {
        return order.deleteNode((Dish)item);
    }

    public int removeAll(Item item) {
        boolean isPresent = order.deleteNode((Dish)item);
        int deleteCount = 0;

        while (isPresent) {
            deleteCount++;
            isPresent = order.deleteNode((Dish)item);
        }
